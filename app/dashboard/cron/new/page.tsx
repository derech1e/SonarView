...
export default function CreateCronJob() {
    ...
    return (
        <form className={"flex flex-col w-full mt-4 gap-4 space-between"} action={formAction}>
            ...
            <InputField id={"from"} label={"From"} name={"startTime"} required={true} type={"time"}/>
            <InputField id={"to"} label={"To"} name={"endTime"} required={true} type={"time"}/>
            <SubmitButton/>

            {error.isVisible &&
                <DialogPopup initValues={{title: "Failed to create job!", message: error.message, submitText: "Got it!", isVisible: true}} onClose={() => setError({...error, isVisible: false})} />
            }
        </form>
    );
}